$(document).ready(function () {
    ...
    function renderizarTabela(consultas) {
            ...
            <table class="todos-table">
                <thead>
                    <tr>
                        ...
                        <th class="th-data">Data e Hora</th>
                        <th class="th-valor">Valor</th>
                        <th class="th-pago">Pago</th>
                        <th> </th>
                    </tr>
                </thead>
                <tbody>
        ...
        consultas.forEach(function (consulta, index) {
            ...
            const pagoTexto = consulta.pago ? 'Sim' : 'Não';
            ...
            tabela += `
                <tr>
                    ...
                    <td class="pago-consulta">${pagoTexto}</td>
                    <td class="pago-consulta"> </td>
                </tr>
            `;

            if (index < consultas.length - 1) {
                tabela += `<tr><td colspan="7" style="height: 10px;"></td></tr>`;
            }
        });
    ...
    function atualizarResumo(consultas) {
        const valorTotal = consultas.reduce((total, consulta) => total + consulta.valor, 0).toFixed(2).replace('.', ',');
        const valorPendente = consultas.filter(consulta => !consulta.pago).reduce((total, consulta) => total + consulta.valor, 0).toFixed(2).replace('.', ',');
        const valorRecebido = consultas.filter(consulta => consulta.pago).reduce((total, consulta) => total + consulta.valor, 0).toFixed(2).replace('.', ',');
        
        const resumo = `
            <div class="dashboard">
                <div class="dashboard-titulo"> <i class="uil uil-money-bill"></i> Valor Total </div> 
                <div class="dashboard-quantidade"> R$${valorTotal} </div>
            </div>
            <div class="dashboard">
                <div class="dashboard-titulo"> <i class="uil uil-money-bill-slash"></i> Valor Pendente </div> 
                <div class="dashboard-quantidade"> R$${valorPendente} </div> 
            </div>
                        <div class="dashboard">
                <div class="dashboard-titulo"> <i class="uil uil-money-withdraw"></i> Valor Recebido </div> 
                <div class="dashboard-quantidade"> R$${valorRecebido} </div> 
            </div>
        `;
    ...
    function exportarExcel() {
        ...
        const ws = XLSX.utils.json_to_sheet(consultasFiltradas.map(consulta => ({
            ...
            'Data e Hora': formatarData(consulta.data) + '-' + consulta.hora.slice(0, 5),
            'Valor': `R$ ${consulta.valor.toFixed(2)}`,
            'Pago': consulta.pago ? 'Sim' : 'Não',
        })));
    ...
    function exportarPDF() {
        ...
        imgIcone.onload = function () {
            ...
            const cabecalhos = ['Motivo', 'Paciente', 'Profissional', 'Data e Hora', 'Valor', 'Pago'];
            const linhas = consultasFiltradas.map(consulta => [
                consulta.motivo,
                consulta.paciente_nome,
                consulta.profissional_nome,
                formatarData(consulta.data) + '-' + consulta.hora.slice(0, 5),
                `R$ ${consulta.valor.toFixed(2)}`,
                consulta.pago === 1 ? 'Sim' : 'Não'
            ]);
